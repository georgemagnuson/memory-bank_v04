...
class MemoryBankDatabase:
    ...
    async def get_database_stats(self) -> Dict[str, Any]:
        """
        Get comprehensive database statistics for both v1.4.0 and v2.0 schemas
        
        v1.4.0: Uses unified_documents + legacy tables
        v2.0: Uses documents table with document_type field
        """
        async with aiosqlite.connect(self.db_path) as db:
            stats = {}
            
            # Check if we're using v2.0 schema (documents table) or v1.4.0 (unified_documents)
            cursor = await db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('documents', 'unified_documents')")
            table_results = await cursor.fetchall()
            table_names = [row[0] for row in table_results]
            
            has_documents_v2 = 'documents' in table_names
            has_unified_documents = 'unified_documents' in table_names
            
            if has_documents_v2:
                # v2.0 Schema: Count by document_type in documents table
                document_types = ['discussion', 'artifact', 'plan', 'code', 'note', 'chat_session']
                
                for doc_type in document_types:
                    cursor = await db.execute(
                        "SELECT COUNT(*) FROM documents WHERE project_uuid = ? AND document_type = ?",
                        (self.project_uuid, doc_type)
                    )
                    count = await cursor.fetchone()
                    stats[f'documents_{doc_type}_count'] = count[0] if count else 0
                
                # Total documents count
                cursor = await db.execute("SELECT COUNT(*) FROM documents WHERE project_uuid = ?", (self.project_uuid,))
                count = await cursor.fetchone()
                stats['documents_total_count'] = count[0] if count else 0
                
                # Maintain backward compatibility - map to legacy names for existing code
                stats['discussions_count'] = stats['documents_discussion_count']
                stats['artifacts_count'] = stats['documents_artifact_count']
                stats['chat_sessions_count'] = stats['documents_chat_session_count']
                stats['plans_count'] = stats['documents_plan_count']
                stats['code_iterations_count'] = stats['documents_code_count']
                
            elif has_unified_documents:
                # v1.4.0 Schema: Count by document_type in unified_documents table
                document_types = ['discussion', 'artifact', 'plan', 'code', 'note', 'chat_session']
                
                for doc_type in document_types:
                    cursor = await db.execute(
                        "SELECT COUNT(*) FROM unified_documents WHERE project_uuid = ? AND document_type = ?",
                        (self.project_uuid, doc_type)
                    )
                    count = await cursor.fetchone()
                    stats[f'documents_{doc_type}_count'] = count[0] if count else 0
                
                # Total unified documents count
                cursor = await db.execute("SELECT COUNT(*) FROM unified_documents WHERE project_uuid = ?", (self.project_uuid,))
                count = await cursor.fetchone()
                stats['documents_v2_count'] = count[0] if count else 0
                
                # Also check legacy tables if they still exist for backward compatibility
                legacy_tables = ['discussions', 'code_iterations', 'artifacts', 'chat_sessions']
                ...
                # Fallback: Legacy v1.x schema only
                legacy_tables = ['discussions', 'code_iterations', 'artifacts', 'chat_sessions']
                for table in legacy_tables:
                    try:
                        cursor = await db.execute(f"SELECT COUNT(*) FROM {table} WHERE project_uuid = ?", (self.project_uuid,))
            ...
            # Cross references (consistent across versions)
            try:
                ...
                stats['cross_references_count'] = 0
    ...
    async def is_empty(self) -> bool:
        ...
        Works with both v1.4.0 (unified_documents) and v2.0 (documents) schemas
        """
        stats = await self.get_database_stats()
        
        # Check v2.0 document counts first
        if 'documents_total_count' in stats:
            return stats['documents_total_count'] == 0
        
        # Check v1.4.0 unified documents count
        if 'documents_v2_count' in stats:
            return stats['documents_v2_count'] == 0
            
        # Fallback to legacy table counts
        return (stats.get('discussions_count', 0) == 0 and 
                stats.get('code_iterations_count', 0) == 0 and 
                stats.get('artifacts_count', 0) == 0)
