...
async def work_on_project(project_path: str) -> str:
    """
    Enhanced work_on_project with registry integration and automatic tool initialization
    
    This tool:
    1. Uses registry system to properly manage context
    2. Initializes all tools with shared context
    3. Fixes context propagation issues
    4. Maintains backwards compatibility with existing tools
    """
    try:
        # Use registry-enhanced work_on_project
...
async def memory_bank_help() -> str:
    ...
    return """
🚀 **Memory Bank v2.1 Enhanced - FTS5 + SEMANTIC SEARCH EDITION**

**✨ STATUS: Revolutionary search capabilities now LIVE!**
• ✅ FTS5 Full-Text Search: Lightning-fast, relevance-ranked results
• ✅ Semantic Query Expansion: Intelligent term relationship discovery
• ✅ Importance-Weighted Ranking: Priority-aware content surfacing
• ✅ Context-Aware Search: Domain-specific semantic understanding
• ✅ Graceful Fallback: Always provides results, never fails

**🏁 GETTING STARTED:**
• `work_on_project("/path/to/project")` - Initialize project with registry context
• `get_current_context()` - Check active project status  
• `get_registry_status()` - Check registry and context status
...
def main():
    ...
    logger.info("🚀 Starting Memory Bank v2.0 Enhanced MCP Server with Registry Integration")
    ...
    try:
        ...
        # Use FastMCP's built-in run method for stdio (like original)
